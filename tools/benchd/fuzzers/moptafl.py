...
class MOptAFLFuzzer(Fuzzer):
    def __init__(self, install_dir):
        super().__init__()

        self.install_dir = install_dir

        self.cc = os.path.join(self.install_dir, "fuzzer", "afl-gcc")
        self.cxx = os.path.join(self.install_dir, "fuzzer", "afl-g++")
        self.ass = os.path.join(self.install_dir, "fuzzer", "afl-as")
        self.fuzz = os.path.join(self.install_dir, "fuzzer", "afl-fuzz")
    ...
    def launch(self, target, seeds_dir, findings_dir, args=None, timeout=86400, logfile=None):
        fuzz_cmd = "{fuzz} -L 0 -i {seeds_dir} -o {findings_dir}{args} -- {target_path} {target_args}".format(
                fuzz = self.fuzz,
                seeds_dir = seeds_dir,
        ...
        name = "moptafl.%d" % randint(10000,99999)
        args = [
            "/usr/bin/env",
            "screen",
            "-S",
...
class MOptAFLBenchmark(FuzzerBenchmark):
    pass
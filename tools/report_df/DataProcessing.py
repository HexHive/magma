...
def expected_time_to_trigger_data(bd) :
    ...
    def expected_time_to_bug(N,M,t):
        ...
        ett = ((M * t) + N_minus_M * (bd.get_campaign_duration() / lambda_t)) / N
        return ett


    df = bd.get_frame()
    df_triggered = df[df.index.get_level_values('Metric') == Metric.TRIGGERED.value]
    t = df_triggered.groupby(['Fuzzer','Library','Program','BugID'])['Time'].mean()
    M = df_triggered.groupby(['Fuzzer','Library','Program','BugID'])['Time'].count()
    N_raw = df.reset_index().groupby(['Fuzzer', 'Library', 'Program'])['Campaign'].nunique()
    ...
    prog_bug = prog_bug.loc[prog_bug.groupby(['Fuzzer','Library','BugID'])['Time'].idxmin()]
    prog_bug = prog_bug.set_index(['Fuzzer','Library','Program','BugID'])

    #Computing the average time to trigger bug over all fuzzers and libraries including all programs
    M_agg = df_triggered.groupby(['Fuzzer','Library','Program','BugID'])['Time'].count()
    t_agg = df_triggered.groupby(['Fuzzer','Library','Program','BugID'])['Time'].sum()
    ...
    def count_campaigns(group):
        target = next(iter(group.groupby(['Library', 'Program']).groups.keys()))
        sum = 0
...
def unique_bugs_per_library_data(bd, metric):
    """
    ...
    unique_bugs = df.reset_index().groupby(['Fuzzer','Library','Campaign'])['BugID'].nunique()
    ...
    agg = unique_bugs.groupby(['Fuzzer', 'Library']) \
                     .apply(lambda d: pd.DataFrame([d.mean(), d.std()],
                                                   index=['Mean', 'Std']) \
                                        .unstack()
                      )

    p_values = unique_bugs.groupby('Library') \
                          .apply(lambda d: compute_p_values(
                                            d.reset_index('Library', drop=True)
                                           )
                           )
...
def bug_list(bd,fuzzer,library,metric):
    """
    ...
    :param library: { chosen library/target }
    :type  library: { string }
...
def line_plot_data(bd,library,metric) :
    """
...
def bug_survival_data(bd):
    def fit_kmf_one(group, supergroup_name, N):
        ...
        T = records.fillna(bd.get_campaign_duration())
        E = records.notnull()
    ...
    def fit_kmf_all(group, N):
        def fillmissing(group, supergroup_name):
            library, program, bug = supergroup_name
            fuzzer = group.name
    ...
    df = bd.get_frame()
    N = df.reset_index().groupby(['Fuzzer', 'Library', 'Program'])['Campaign'].nunique()
    kmf = df.reset_index() \
            .groupby(['Library', 'Program', 'BugID']) \
            .apply(fit_kmf_all, N)

    means = kmf.applymap(lambda k: restricted_mean_survival_time(k, bd.get_campaign_duration()))
    means = means.stack().stack()
    means.rename('Time', inplace=True)
    means = means.reset_index()
    means = means.drop(columns=['Library', 'Program']).groupby(['BugID', 'Fuzzer', 'Metric']).min()

...
echo "corpus minimization tool for afl-fuzz by <lcamtuf@google.com> (fitted for Angora)"
echo
...
unset IN_DIR OUT_DIR STDIN_FILE EXTRA_PAR MEM_LIMIT_GIVEN \
  ANGORA_CMIN_CRASHES_ONLY ANGORA_CMIN_ALLOW_ANY QEMU_MODE

while getopts "+i:o:f:m:t:eC" opt; do
  ...
  case "$opt" in
    ...
    "t")
         ...
         ;;
    "e")
         EXTRA_PAR="$EXTRA_PAR -b"
         ;;
    "C")
         export ANGORA_CMIN_CRASHES_ONLY=1
         ;;
    "?")
         exit 1
         ;;
...
if [ "$ANGORA_PATH" = "" ]; then
  SHOWMAP="${0%/angora-cmin}/target/release/showmap"
else
  SHOWMAP="$ANGORA_PATH/target/release/showmap"
fi

if [ ! -x "$SHOWMAP" ]; then
  echo "[-] Error: can't find 'showmap' - please set ANGORA_PATH." 1>&2
  rm -rf "$TRACE_DIR"
  exit 1
fi
...
# Make sure that we can actually get anything out of showmap before we
# waste too much time.
...
if [ "$STDIN_FILE" = "" ]; then
  ...
  ANGORA_CMIN_ALLOW_ANY=1 "$SHOWMAP" -M "$MEM_LIMIT" -T "$TIMEOUT" -o "$TRACE_DIR/.run_test" -Z $EXTRA_PAR -- "$@" <"$IN_DIR/$FIRST_FILE"

else

  cp "$IN_DIR/$FIRST_FILE" "$STDIN_FILE"
  ANGORA_CMIN_ALLOW_ANY=1 "$SHOWMAP" -M "$MEM_LIMIT" -T "$TIMEOUT" -o "$TRACE_DIR/.run_test" -Z $EXTRA_PAR -A "$STDIN_FILE" -- "$@" </dev/null
...
if [ "$FIRST_COUNT" -gt "0" ]; then
  ...
  test "$ANGORA_KEEP_TRACES" = "" && rm -rf "$TRACE_DIR"
  exit 1
